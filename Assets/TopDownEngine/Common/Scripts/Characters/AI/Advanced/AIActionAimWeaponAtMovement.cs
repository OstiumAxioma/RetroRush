...
namespace MoreMountains.TopDownEngine
{
	...
	public class AIActionAimWeaponAtMovement : AIAction
	{
		...
		private void Update()
		{
			// Debug.Log("[AIMovement] Shooting?" + Shooting());
			if (!Shooting())
			{
				UpdateAim();
			}
			if (this.gameObject.GetComponent<AIBrain>().CurrentState.StateName == "MoveTowardsTarget") {
                NormalShootingStrategy();
            }
        }

		private void NormalShootingStrategy() 
		{
			if (_characterHandleWeapon == null)
			{
				...
				//Debug.Log(_characterHandleWeapon.name);
