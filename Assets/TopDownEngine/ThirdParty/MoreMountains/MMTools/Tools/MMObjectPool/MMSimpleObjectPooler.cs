	...
	public class MMSimpleObjectPooler : MMObjectPooler 
	{
		...
		public override GameObject GetPooledGameObject()
		{
			...
			for (int i=0; i< _objectPool.PooledGameObjects.Count; i++)
			{
				...
				{
					string lastName = _objectPool.PooledGameObjects[i].gameObject.name;
					GameObject temp = _objectPool.PooledGameObjects[i].gameObject;
                    _objectPool.PooledGameObjects.RemoveAt(i);
                    Destroy(temp);
                    GameObject newGameObject = (GameObject)Instantiate(GameObjectToPool);
					newGameObject.name = lastName;
                    // if we find one, we return it
                    _objectPool.PooledGameObjects.Insert(i, newGameObject);
